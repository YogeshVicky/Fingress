...
import com.fingress.googleSheets.transformer.GoogleSheetsTransformer;

@Component
public class GoogleSheetsRouter {
		...
		context.addRoutes(new RouteBuilder() {
			...
			public void configure() throws Exception {
				...
				from("timer:run?repeatCount=1")
						...
						.convertBodyTo(String.class).process(new GoogleSheetsTransformer())
 ...
 * getContext().addComponent("google-sheets", component);
 * 
 * from("timer:run?repeatCount=1") .to(
 * "google-sheets:data/get?spreadsheetId=1i8jAVG3r6BINzdU0NzgAIuDUuz3FiVRyFF1Wm6WQq_0&range='Main'")
 * .log("${body}").convertBodyTo(String.class).process(new
 * TransformerGoogleSheets()).to("file:file/Input?fileName=employee.csv").log(
 * "Sucess");
 * 
 * }
 */
